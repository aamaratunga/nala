...
final class EventFileWatcher: @unchecked Sendable {
    private let logger = os.Logger(subsystem: "com.coral.app", category: "EventFileWatcher")

    /// Base directory for event JSONL files
    static let eventsDirectory: String = {
        let home = FileManager.default.homeDirectoryForCurrentUser.path
        return "\(home)/.coral/events"
    }()
    ...
    private let watcherQueue = DispatchQueue(label: "com.coral.eventfilewatcher", qos: .utility)
    ...
    static func parseHookEvent(_ json: [String: Any]) -> (eventType: String, summary: String, waitingReason: String?, waitingSummary: String?)? {
        ...
        if hookType == "SessionStart" || json["_coral_session_clear"] != nil {
