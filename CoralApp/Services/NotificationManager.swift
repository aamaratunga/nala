...
final class NotificationManager: NSObject, UNUserNotificationCenterDelegate {
    ...
    func evaluateTransition(old: Session?, new: Session) {
        ...
        if new.waitingForInput && !wasWaiting {
            let enabled = defaults.object(forKey: "coral.notifications.needsInput") as? Bool ?? true
            if enabled {
        ...
        // Done: false → true
        if new.done && !wasDone {
            let enabled = defaults.object(forKey: "coral.notifications.done") as? Bool ?? true
            if enabled {
