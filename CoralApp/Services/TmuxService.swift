...
final class TmuxService: @unchecked Sendable {
    private let logger = os.Logger(subsystem: "com.coral.app", category: "TmuxService")
    ...
    /// Regex for Coral-managed session names: {agentType}-{uuid}
    ...
    init() {
        ...
        if let found = Self.knownTmuxPaths.first(where: { fm.isExecutableFile(atPath: $0) }) {
            ...
            os.Logger(subsystem: "com.coral.app", category: "TmuxService")
    ...
    /// Parse a tmux session name into agent type and UUID if it matches Coral format.
    ...
    /// Lists all Coral-managed tmux sessions.
    ...
    /// Create a new Coral-managed tmux session and return the session name.
    func createSession(
        agentType: String,
        ...
        let logPath = "/tmp/coral_\(sessionName).log"
    ...
    func killSession(name: String) async {
        ...
        // Remove log file
        let logPath = "/tmp/coral_\(name).log"
        try? FileManager.default.removeItem(atPath: logPath)

        // Remove settings temp file
        if let parsed = Self.parseSessionName(name) {
            let settingsPath = "/tmp/coral_settings_\(parsed.uuid).json"
            try? FileManager.default.removeItem(atPath: settingsPath)
            let promptPath = "/tmp/coral_prompt_\(parsed.uuid).txt"
    ...
    /// Build merged Claude settings with Coral hooks injected.
    /// The sessionId is used to generate file-based hook commands that write
    /// raw hook JSON to ~/.coral/events/{sessionId}.jsonl for EventFileWatcher.
    static func buildMergedSettings(workingDirectory: String, sessionId: String) -> [String: Any] {
        ...
        // Coral hooks to inject
        let coralHooks: [(String, [String: Any])] = [
            ("PostToolUse", [
    ...
    func buildClaudeLaunchCommand(
        ...
        let settingsPath = "/tmp/coral_settings_\(sessionId).json"
        ...
        if let prompt, !prompt.isEmpty {
            let promptPath = "/tmp/coral_prompt_\(sessionId).txt"
    ...
    func ensurePipePane(session: TmuxSessionInfo) async {
        let logPath = "/tmp/coral_\(session.sessionName).log"
